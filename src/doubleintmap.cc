...
namespace PlotMM {
  ...
  double DoubleIntMap::inv_transform(int y) const
  {
    ...
    return (d_log) ? exp(d_x1 + static_cast<double>(y - d_y1) / d_cnv )
      : d_x1 + static_cast<double>(y - d_y1) / d_cnv;
  }
  ...
  double DoubleIntMap::x_transform(double x) const
  {
    ...
    rv = d_log ? static_cast<double>(d_y1) + (log(x) - d_x1) * d_cnv
      : static_cast<double>(d_y1) + (x - d_x1) * d_cnv;
    return rv;
  }
  ...
  {
    d_cnv = (d_x2 != d_x1) ? static_cast<double>(d_y2 - d_y1) / (d_x2 - d_x1)
      : 0.0;
  }
