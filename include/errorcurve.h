  ...
  class ErrorCurve : public Curve
  {
    public:
      ...
      virtual void draw(
          ...
          const Glib::RefPtr<Gdk::Window> painter,
    ...
    protected:
      ...
      virtual void draw_x_error_(
          const Cairo::RefPtr<Cairo::Context> &cr,
          const Glib::RefPtr<Gdk::Window> p,
          int x1, int y1, int x2, int y2);
      virtual void draw_y_error_(
          const Cairo::RefPtr<Cairo::Context> &cr,
          const Glib::RefPtr<Gdk::Window> p,
          int x1, int y1, int x2, int y2);

      bool have_dx_() const {
        return data_size() && (static_cast<int>(dx_.size()) == static_cast<int>(data_size()));
      }
      bool have_dy_() const {
        return data_size() && (static_cast<int>(dy_.size()) == static_cast<int>(data_size()));
      }
    private:
